...
export default function VideoHome(active) {
    ...
    const toProfile = () => {
      ...
      const searchQuery= async () => {
        ...
        console.log(query);
        axios.get(`${host}/api/v1/search/video/?videoname=${query}&deleted=false`,{
            headers: {
                'Authorization': `Bearer ${token}`
            },
        })
...
<nav className='p-5 flex justify-between  bg-gradient-to-r from-indigo-500 to-blue-500 absolute top-0 -left-1 w-full mb-5' >
        ...
        <image src={'https://github.com/sasta-notion/Note-ed-Dashboard/blob/master/src/assets/icon.png'} alt='logo' /> 
